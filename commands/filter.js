  ...
  run: async (client, interaction) => {
    try {
      ...
      let embed = new EmbedBuilder()
        ...
        .setDescription(
          "** Explore the Beat, Choose Your Sound Magic Below!**"
        );
      ...
      interaction
        ...
        .then(async (Message) => {
          ...
          col.on("collect", async (button) => {
            ...
            if (!filtre)
              return interaction
                ?.editReply({ content: "❌ Invalid Name", ephemeral: true })
                .catch((e) => {});
            ...
            if (filters?.includes(filtre?.toLowerCase())) {
              if (queue?.filters?.has(filtre)) {
                ...
                embed?.setDescription(
                  `Magic : **{filter}**, Applied Status: **{status}**`
