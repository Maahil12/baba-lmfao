    ...
    try {
      ...
      if (stp === "create") {
        ...
        if (!name)
          return interaction
            .reply({
              content: "⚠️ Enter Album name to create!",
              ephemeral: true,
            })
            .catch((e) => {});
        ...
        if (playlist?.length > 0) {
          for (let i = 0; i < playlist.length; i++) {
            if (
              ...
              return interaction
                .reply({ content: "⚠️ Album already Exitst!", ephemeral: true })
                .catch((e) => {});
            }
          }
        }
        ...
        if (
          ...
          return interaction
            .reply({ content: "🚫 Exceeded Album limit", ephemeral: true })
            .catch((e) => {});

        await interaction
          .reply({ content: `<@${interaction.member.id}>, 🎸 Creating Album!` })
          .catch((e) => {});
        ...
        await interaction
          .editReply({
            content: `<@${interaction.member.id}>, ✅ Album Created Sucessfully`,
          })
          .catch((e) => {});
      }
      ...
      if (stp === "delete") {
        ...
        if (!playlist?.playlist?.filter((p) => p.name === name).length > 0)
          return interaction
            .reply({ content: "❌ No album Found", ephemeral: true })
            .catch((e) => {});
        ...
        await interaction
          .reply({ content: `<@${interaction.member.id}>, Deleting Album..` })
          .catch((e) => {});
        ...
        await interaction
          .editReply({
            content: `<@${interaction.member.id}>, 🚫 Album Deleted!`,
          })
          .catch((e) => {});
      }
      ...
      if (stp === "add-music") {
        ...
        if (!name)
          return interaction
            .reply({ content: "⚠️ Enter song name to search", ephemeral: true })
            .catch((e) => {});
        let playlist_name = interaction.options.getString("playlist-name");
        if (!playlist_name)
          return interaction
            .reply({
              content: "⚠️ Enter album name to add songs",
              ephemeral: true,
            })
            .catch((e) => {});
        ...
        if (
          ...
          return interaction
            .reply({ content: "Your Song Added!", ephemeral: true })
            .catch((e) => {});
        ...
        if (
          ...
          return interaction
            .reply({
              content: "Reached Album songs limit".replace(
                "{max_music}",
                max_music
              ),
              ephemeral: true,
        ...
        try {
          ...
          return interaction
            .reply({ content: "Cannod Find ❌", ephemeral: true })
            .catch((e) => {});
        }
        if (!res || !res.length || !res.length > 1)
          return interaction
            .reply({ content: `Cannot Find ❌ `, ephemeral: true })
            .catch((e) => {});

        await interaction
          .reply({ content: `<@${interaction.member.id}>, 🔴 Loading Songs!` })
          .catch((e) => {});
        ...
        await interaction
          .editReply({
            content: `<@${interaction.member.id}>, \`${res[0]?.name}\` Playing Now!`,
          })
          .catch((e) => {});
      }
      ...
      if (stp === "delete-music") {
        ...
        if (!name)
          return interaction
            .reply({
              content: "⚠️ Enter Song Name to Search!",
              ephemeral: true,
            })
            .catch((e) => {});
        let playlist_name = interaction.options.getString("playlist-name");
        if (!playlist_name)
          return interaction
            .reply({
              content: "⚠️ Enter name of the album to remove song!",
              ephemeral: true,
            })
            .catch((e) => {});
        ...
        if (!music_filter?.length > 0)
          return interaction
            .reply({ content: `❌ No Song found!`, ephemeral: true })
            .catch((e) => {});

        await interaction
          .reply({ content: `<@${interaction.member.id}>, Removing Song!}` })
          .catch((e) => {});
        ...
        await interaction
          .editReply({ content: `<@${interaction.member.id}>, Song Removed!` })
          .catch((e) => {});
      }
      ...
      if (stp === "list") {
        ...
        if (!name)
          return interaction
            .reply({
              content: "⚠️ Enter Album name to find it!",
              ephemeral: true,
            })
            .catch((e) => {});
        ...
        if (!playlist?.length > 0)
          return interaction
            .reply({ content: `🚫 No Album name!`, ephemeral: true })
            .catch((e) => {});
          ...
          if (
            ...
            if (playlist_owner_filter !== interaction.member.id) {
              if (playlist_public_filter === false) {
                return interaction
                  .reply({
                    content: "🚫 You cannot play this Album!",
                    ephemeral: true,
                  })
                  .catch((e) => {});
              }
            ...
            if (!trackl?.length > 0)
              return interaction
                .reply({ content: "❌ No Music", ephemeral: true })
                .catch((e) => {});
          } else {
            arr++;
        ...
        const generateEmbed = async (start) => {
          ...
          if (!current || !current?.length > 0)
            return interaction
              .reply({
                content: "❌ Your album is Empty, add any songs to it!",
                ephemeral: true,
              })
              .catch((e) => {});
          return new EmbedBuilder()
            ...
            .setDescription(
              `Use the **/play playlist <list-name>** command to play.\nType **/playlist list <list-name>** to see the songs\n${current.map(
                (data) =>
          ...
          .then(async (Message) => {
            ...
            collector.on("collect", async (button) => {
              if (button.customId === "close") {
                collector.stop();
                return button
                  .reply({ content: `Command Cancelled ❌`, ephemeral: true })
                  .catch((e) => {});
              } else {
            ...
            collector.on("end", async (button) => {
              ...
              const embed = new EmbedBuilder()
                ...
                .setColor(client.config.embedColor)
                .setDescription(
                  "Timeout Use command again!".replace("{name}", name)
                )
                .setFooter({ text: "Arox" });
              return interaction
      ...
      if (stp === "lists") {
        ...
        if (!playlist?.playlist?.length > 0)
          return interaction
            .reply({
              content: `⚠️ You haven't created a playlist`,
              ephemeral: true,
            })
            .catch((e) => {});
              ...
              (data) =>
                `\n**${number++} |** \`${data.name}\` - **${
                  playlist?.musics?.filter((m) => m.playlist_name === data.name)
                    ?.length || 0
                }** plays (<t:${Math.floor(data.createdTime / 1000)}:R>)`
            )}`
          )
          .setFooter({ text: "😼" });
      ...
      if (stp === "top") {
        ...
        if (!playlists?.length > 0)
          return interaction
            .reply({ content: "There are no playlists ❌", ephemeral: true })
            .catch((e) => {});
          ...
          .then(async (Message) => {
            ...
            collector.on("collect", async (button) => {
              if (button.customId === "close") {
                collector.stop();
                return button
                  .reply({ content: `Command stopped ✅`, ephemeral: true })
                  .catch((e) => {});
              } else {
                if (button.customId === backId) {
                  page--;
